			...
			switch (segment.type) {
				...
				case segment_type::PT_LOAD: {
					...
					for (auto offset = 0uz; offset < segment.size_in_memory; offset += 0x1000) {
						auto mmap_returned = threads::local_scheduler->get_current_task()
						                             ->new_mmap(aligned_start.address + offset, 0x1000, false);
						if (mmap_returned != aligned_start.address + offset)
							dyld_terminate("Couldn't load segment at required address");
					}
