...
int Elf64FileHeader::verify_header() {
	if (this->e_ident[0] != ELF_MAGIC[0] || this->e_ident[1] != ELF_MAGIC[1] || this->e_ident[2] != ELF_MAGIC[2] || this->e_ident[3] != ELF_MAGIC[3]) {
		return -1;
	}
	if (this->e_ident[4] != ELF_CLASS_64) {
		return -1;
	}
	if (this->e_ident[5] != ELF_DATA_LITTLE_ENDIAN) {
		return -1;
	}
	if (this->e_ident[6] != ELF_VERSION_CURRENT) {
		return -1;
	}
	if (this->e_ident[7] != ELF_OSABI_UNIX_SYSTEM_V) {
		return -1;
	}
	if (this->e_type != ET_EXEC) {
		return -1;
	}
	if (this->e_machine != ELF_MACHINE_AMD64) {
		return -1;
	}
	return 0;
}
