...
public:
	...
	constexpr optional(const optional<T>& other) requires(std::is_copy_constructible_v<T>) : is_some(
				other.is_some) {
		if (this->is_some) new(&this->inner) T(other.inner);
	}
	constexpr optional(const optional<T>& other) requires(!std::is_copy_constructible_v<T>) = delete;
	...
	constexpr optional(const optional<T>&& other) noexcept requires(!std::is_move_constructible_v<T>) = delete;

	template<class... Args> constexpr explicit optional(std::in_place_t, Args&& ... args) : is_some(true) {
		new(&this->inner) T(args...);
	}

	template<class U = T> constexpr optional(U&& value) requires(!std::same_as<std::remove_reference_t<U>, optional<T>> && !std::same_as<std::remove_reference_t<U>, std::in_place_t>) : is_some(true) {
...
public:
	...
	template<class F> constexpr optional or_else(F&& f)&& {
		return *this ? move(*this) : std::forward<F>(f)();
	}
