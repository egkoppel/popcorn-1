...
public:
	...
	constexpr optional(U&& value)
		requires(!std::same_as<std::remove_reference_t<U>, optional<T>>
	...
	template<class F> constexpr optional or_else(F&& f) && { return *this ? std::move(*this) : std::forward<F>(f)(); }
...
template<class T> constexpr optional<T> make_optional(T&& value) { return optional<T>(std::forward<T>(value)); }
