...
template<class U, class... Args> unique_ptr<U> make_unique(Args&&...args) {
	return unique_ptr<U>(new U(forward<Args>(args)...));
}
...
template<class T> bool operator==(const unique_ptr<T>& lhs, decltype(nullptr)) noexcept { return !lhs.get(); }
...
template<class T, class... Args> shared_ptr<T> make_shared(Args&&...args) { return shared_ptr<T>(new T(args...)); }
...
template<class T> bool operator==(const shared_ptr<T>& lhs, decltype(nullptr)) noexcept { return !lhs.get(); }
