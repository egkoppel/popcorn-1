...
psf_copychar: ; (char_index, y, x)
	...
	cmp eax, [psf_numglyph - KERNEL_VIRTUAL_BASE]
	jle .valid_index
	jmp .end
.valid_index:
	
	imul dword [psf_bytesperglyph - KERNEL_VIRTUAL_BASE] ; clobbers edx
	add eax, font_psf_start - KERNEL_VIRTUAL_BASE
	add eax, [psf_headersize - KERNEL_VIRTUAL_BASE] ; eax now points to the relevant char in the PSF
	push eax
	
	mov eax, [framebuffer_pitch - KERNEL_VIRTUAL_BASE]
	...
	movzx edi, byte [framebuffer_bpp - KERNEL_VIRTUAL_BASE]
	shr edi, 3
	imul edi ; eax is now the horizontal offset into the framebuffer
	mov edx, [psf_width - KERNEL_VIRTUAL_BASE]
	inc edx
	imul edx
	
	mov esi, [framebuffer_addr - KERNEL_VIRTUAL_BASE]
	...
	mov ecx, 0 ; y counter
.loop_y_begin:
	cmp ecx, [psf_height - KERNEL_VIRTUAL_BASE]
	jge .loop_y_end
	
	mov ebx, 0 ; x counter
.loop_x_begin:
	cmp ebx, [psf_width - KERNEL_VIRTUAL_BASE]
	jge .loop_x_end
	
	mov esi, [esp] ; esi = fb_dest
	mov eax, ecx ; eax = y coord offset
	imul dword [framebuffer_pitch - KERNEL_VIRTUAL_BASE]
	...
	mov eax, ecx ; y
	imul dword [psf_width - KERNEL_VIRTUAL_BASE]
	...
	mov edx, 0
	test ax, 1<<7
	cmovnz edx, [col - KERNEL_VIRTUAL_BASE]
