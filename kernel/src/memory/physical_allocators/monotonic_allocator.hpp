...
namespace memory::physical_allocators {
	...
	private:
		...
		aligned<paddr_t> ramdisk_start_frame;
		aligned<paddr_t> ramdisk_end_frame;
	...
	public:
		MonotonicAllocator(paddr_t start_addr,
			...
			: next_frame(aligned<paddr_t>::aligned_up(start_addr)),
			  ...
			  ramdisk_start_frame(aligned<paddr_t>::aligned_down(ramdisk_start)),
			  ramdisk_end_frame(aligned<paddr_t>::aligned_up(ramdisk_end)),
			  mem_map(mem_map) {}
		...
		aligned<paddr_t> get_ramdisk_start_frame() const { return this->ramdisk_start_frame; }
		aligned<paddr_t> get_ramdisk_end_frame() const { return this->ramdisk_end_frame; }
