...
namespace memory {
	frame_t *IPhysicalAllocator::allocate(u64 size) {
		LOG(Log::DEBUG, "Physical allocation requested of size %llx, from allocator %s", size, this->name());
	...
	frame_t *IPhysicalAllocator::allocate(aligned<paddr_t> at, u64 size) {
		LOG(Log::DEBUG, "Physical allocation requested of size %llx at %lp, from allocator %s", size, at, this->name());
