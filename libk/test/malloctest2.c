...
static void hexdump(const unsigned char *data, const unsigned char *cmp, const size_t len) {
	...
	while (i < len) {
		if (cmp != NULL && data[i] != cmp[i]) { printf(TERMCOLOR_RED); }

		printf("%02x ", data[i]);

		if (cmp != NULL && data[i] != cmp[i]) { printf(TERMCOLOR_RESET); }
	...
	if (len % HEXDUMP_BYTES_PER_LINE
	    != 0) {   // if len is a multiple of HEXDUMP_BYTES_PER_LINE we already printed a newline
		printf("\n");
	} else {
...
void test_malloc2(unsigned int random_seed) {
	...
	for (size_t j = 0; j < ALLOCS1 + ALLOCS2; ++j) {
		cprintf("Checking string %5lu of length %5lu... [libc = %18p, hug = %18p]",
		        j,
		        duals[j].size,
		        (void *)duals[j].libc,
		        (void *)duals[j].hug);
